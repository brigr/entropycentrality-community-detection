...
for t=1:length(tvals)
    for c=2:size(M,1)
        disp(['computing KAPFMM partition with c=' int2str(c) ' communities.']);
        ...
        if isempty(communities{t,c-1}.community_indices)
            ...
            % write communities
            csvwrite(strcat('KAPFMM_mod_t',int2str(t),'_c',int2str(c),'_communities.csv'),communities{t,c-1}.community_indices);
            
            % write node centralities
            csvwrite(strcat('KAPFMM_mod_t',int2str(t),'_c',int2str(c),'_centralities.csv'),communities{t,c-1}.node_centralities);
            
            % write modularity
            csvwrite(strcat('KAPFMM_mod_t',int2str(t),'_c',int2str(c),'_modularity.csv'),modularities{t,c-1});
        end
    end    
end
