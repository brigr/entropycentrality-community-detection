...
M = load('Sawmill.edgelist');

communities = cell(5,35);
modularities = cell(5,35);
...
for t=1:5
    for c=2:36
        ...
        if isempty(communities{t,c-1}.community_indices)
            ...
            % write communities
            csvwrite(strcat('sawmill_mod_t',int2str(t),'_c',int2str(c),'_communities.csv'),communities{t,c-1}.community_indices);
            
            % write node centralities
            csvwrite(strcat('sawmill_mod_t',int2str(t),'_c',int2str(c),'_centralities.csv'),communities{t,c-1}.node_centralities);
            
            % write modularity
            csvwrite(strcat('sawmill_mod_t',int2str(t),'_c',int2str(c),'_modularity.csv'),modularities{t,c-1});
        end
    end    
end
